...
final class SavedLightStoreStateFactory
{
    ...
    //? if >=1.21.11 {
	    /*private static final Codec<SavedLightStore> CODEC = NbtCompound.CODEC.xmap(
	            ...
	            SavedLightStoreStateBridge::write);

    static PersistentStateType<SavedLightStore> type(String id)
    {
        return new PersistentStateType<>(id, SavedLightStoreStateBridge::create, CODEC, DataFixTypes.SAVED_DATA_MAP_DATA);
    }
    *///?} elif >=1.20.6 {
    private static final PersistentState.Type<SavedLightStore> FACTORY = new PersistentState.Type<>(SavedLightStoreStateBridge::create, SavedLightStoreStateFactory::load, null);
    ...
    {
        return SavedLightStoreStateBridge.load(tag);
    }
    //?}
}
